...
"""Nordix Hyprland Decorations GUI
...
class DecorationSettings:
    ...
    def __init__(self, config_path=None):
        if config_path is None:
            self.config_path = Path.home() / ".config/nordix/desktop/config/nordix-decorations.conf"
        else:
    ...
    def load_config(self):
        """Reads current settings from config file"""
    ...
    def _set_value(self, key, value, in_blur, in_shadow):
        ...
        if in_blur:
            ...
            key_map = {
                ...
                'sharp': 'shadow_sharp',
                'ignore_window': 'shadow_ignore_window',
                'color': 'shadow_color',
...
source = ~/.config/nordix/system/color/colors.conf
...
# if true, the shadow will not be rendered behind the window itself (bool)
ignore_window = {'true' if s['shadow_ignore_window'] else 'false'}
...
class ColorRow(Adw.ActionRow):
    """A row with dropdown for color selection"""
...
class NordixDecorationsWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, app):
        super().__init__(application=app, title="Nordix Decorations")
    ...
    def setup_ui(self):
        ...
        self.widgets['shadow_ignore_window'] = SwitchRow(
            "Ignore Window", "Don't render shadow behind window itself",
            self.settings.settings['shadow_ignore_window'],
            lambda v: self.on_value_changed('shadow_ignore_window', v)
        )
        shadow_group.add(self.widgets['shadow_ignore_window'])
